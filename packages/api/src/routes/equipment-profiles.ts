...
// name is non-nullable in updates: the column is NOT NULL.
const createProfileBody = {
  type: 'object',
  additionalProperties: false,
  required: ['name', 'availableEquipment'],
  properties: {
...
export async function equipmentProfileRoutes(app: FastifyInstance) {
  ...
  // Create profile
  app.post<{ Body: { name: string; availableEquipment: string[] } }>('/equipment-profiles', { schema: { body: createProfileBody } }, async (req, reply) => {
    const result = db.insert(schema.equipmentProfiles).values(req.body).returning().get();
    return reply.status(201).send(result);
  });

  // Update profile
  app.put<{ Params: { id: string }; Body: { name?: string; availableEquipment?: string[] } }>('/equipment-profiles/:id', { schema: { params: idParams, body: updateProfileBody } }, async (req, reply) => {
    const id = parseInt(req.params.id);
  ...
  app.delete<{ Params: { id: string } }>('/equipment-profiles/:id', { schema: { params: idParams } }, async (req, reply) => {
    const id = parseInt(req.params.id);
